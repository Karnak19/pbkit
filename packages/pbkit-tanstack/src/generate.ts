...
function queryOptions_(col: CollectionSchema, collections?: CollectionsConfig): string[] {
  ...
  if (op("getFirst")) {
    lines.push(
      `export function getFirst${s}Options(pb: PbClient, filter: string, options?: RequestOptions) {`,
    );
    lines.push("  return queryOptions({");
    lines.push(`    queryKey: [${c}, "first", filter],`);
    lines.push(`    queryFn: () => getFirst${s}(pb, filter, options),`);
    lines.push("  })");
    lines.push("}");
    lines.push("");
  }
...
function mutationOptions_(col: CollectionSchema, collections?: CollectionsConfig): string[] {
  ...
  if (op("create")) {
    lines.push(`export function create${s}MutationOptions(pb: PbClient) {`);
  ...
  if (op("update")) {
    lines.push("");
    lines.push(`export function update${s}MutationOptions(pb: PbClient) {`);
    lines.push("  return mutationOptions({");
    lines.push(
  ...
  if (op("delete")) {
    lines.push("");
    lines.push(`export function delete${s}MutationOptions(pb: PbClient) {`);
    lines.push("  return mutationOptions({");
