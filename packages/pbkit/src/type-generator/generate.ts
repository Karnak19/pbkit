...
function fieldDecl(field: CollectionField, options: InternalOptions, collectionName: string): string {
  // A configured json field maps to its explicit type; everything else uses
  // the built-in mapping (json falls back to `unknown`).
  const override = field.type === "json"
    ? getFieldConfig(collectionName, field.name, options.collections)?.type
    : undefined
  const tsType = override ?? fieldTypeToTs(field, options)
...
function recordType(collection: CollectionSchema, options: InternalOptions): string {
...
function getExpandPaths(
  ...
  // can expand `{B}_via_{x}` into an array of B records. Always multi, named per
  // PocketBase's convention. Included in the same depth/visited budget as
  // forward paths so cycles (e.g. listing.reports_via_listing.listing…) terminate.
  for (const rel of ir.relations) {
    ...
    // A forward field literally named `{x}_via_{y}` wins; the expand keyspace is
    // shared, so we can't emit two entries for one key.
...
// are always `multi: true` and keyed `{sourceCollection}_via_{field}` per
// PocketBase's convention. Returns null when there are none.
function relationsMapType(
  ...
  const name = pascalCase(col.name)
  const seen = new Set(rels.map(r => r.fieldName))
  ...
  for (const r of backRels) {
    ...
    entries.push(`  ${key}: { rec: ${rec}; coll: ${coll}; multi: true }`)
  }
...
export function generate(ir: SchemaIR, options: InternalOptions = {}): string {
  ...
  // Imports for configured json field types that reference an external module.
  const jsonImports = new Map<string, Set<string>>()
  for (const col of cols) {
    for (const field of col.fields) {
      if (field.type !== "json") continue
      const fc = getFieldConfig(col.name, field.name, options.collections)
      if (!fc?.from) continue
      if (!jsonImports.has(fc.from)) jsonImports.set(fc.from, new Set())
      jsonImports.get(fc.from)!.add(fc.type)
    }
  }
  if (jsonImports.size > 0) {
    for (const [from, types] of jsonImports) {
      parts.push(`import type { ${[...types].join(", ")} } from ${JSON.stringify(from)}`)
    }
    parts.push("")
  }
